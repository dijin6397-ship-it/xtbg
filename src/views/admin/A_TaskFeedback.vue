    ...
    <div class="card" v-if="task.outputs?.length">
      ...
      <div class="output-list">
        <div v-for="out in task.outputs" :key="out.id" class="output-item" :class="'output-' + out.status">
          ...
          <div v-if="out.scoreRule" class="output-score">
            <span class="score-label">分值：</span>
            <template v-if="out.scoreTotal > 0">
              <span class="score-text">{{ out.scoreValue }} × {{ out.scoreQuantity }} = <strong>{{ out.scoreTotal }}</strong></span>
              <span class="self-score-tag">提交人填报</span>
            </template>
            <span v-else class="score-placeholder">参考分值 {{ refRangeText(out) }}</span>
            <span class="output-rule">({{ ruleLabel(out.scoreRule) }})</span>
          </div>
...
// 参考分值文案：提交人未填报时，列表里显示分类的参考分值而不是 0 × 0 = 0
function refRangeText(out) {
  if (!out) return ''
  if (out.scoreRule === 'fixed') return `≤ ${out.refScoreValue}`
  if (out.scoreRule === 'range') return `${out.refScoreValue} - ${out.refScoreUpper}`
  return '任意'
}
...
async function handleSubmitOutput() {
  ...
  try {
    ...
    // 提交人填写了分值/数量就一并保存，作为主管审核时的「提交人填报值」（所有任务类型均适用）
    if (selfScoreValue.value > 0) {
...
.output-score {
  ...
  border-top: 1px solid #f0f0f0; font-size: 12px; color: var(--text-secondary);
}
.score-label { font-weight: 500; }
.score-text { color: var(--primary); font-weight: 600; }
.score-placeholder { color: var(--text-secondary); }
.output-rule { color: var(--text-caption); }
.self-score-tag {
  display: inline-block; padding: 1px 6px; background: #e6f4ff; color: #1677ff;
